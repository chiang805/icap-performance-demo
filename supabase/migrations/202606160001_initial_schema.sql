...
create table public.ogsm_node (
  ...
  node_type text not null check (node_type in ('O','G','S','M_D','M_P')),
  title text not null,
  description text,
  objective_prompt text,
  smart_goal jsonb not null default '{}',
  strategy_tradeoff text,
  measure_kind text check (measure_kind in ('dashboard','plan') or measure_kind is null),
  owner_unit text,
  collaborator_units jsonb not null default '[]',
  due_date date,
  deliverable text,
